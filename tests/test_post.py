...
from conftest import _TestModelAttrs, KeyVal, get_create_a_post_get_response_safely
...
def test_post(
    ...
    DeletePostTester(
        ...
        qs=item_to_delete_adapter.item_cls.objects.all(), delete_url_addr=del_url_addr
    )
...
def _test_edit_post(
    ...
    edit_url, del_url = find_edit_and_delete_urls(
        ...
        urls_start_with=KeyVal(
            key=post_url.replace(f"/{item_to_edit.id}/", "/<post_id>/"), val=post_url
        ),
    )
    ...
    update_props = {
        item_to_edit_adapter.item_cls_adapter.displayed_field_name_or_value: f"{old_prop_value} edited"
    }
